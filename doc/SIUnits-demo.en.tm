...
<\body>
  ...
  See <hlink|Changelog|./SIUnitsSimple-development-notes.tm>
  ...
  We separate the number from the unit using a space.<todo|Remark the
  different semantics of space and multiplication sign in <TeXmacs>>
  ...
  <strong|SI math> (the following formula is supposed to be inline, but acts
  as a display element; it has a <inactive|<hybrid|math>> tag instead of a
  \<less\>math one.
  ...
  Copy a tree generated by SIScheme inside a math environment

  <math|2 <with|math-font-shape|right|m>*<with|math-font-shape|right|s<rsup|-1>>>

  How should the SI macro behave when it is in nested environments (for
  example, text within math)?

  Here are some nested environments, not yet (2020-11-20) with tests of SI

  <strong|test of equations in bold text <math|a+b=c
  \<frak-D\><with|math-font-shape|right|a> b <math-ss|cd> <text|ef> > \ >
</body>
